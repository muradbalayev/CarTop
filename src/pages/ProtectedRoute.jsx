...
const ProtectedRoute = ({ children, requiredRole }) => {
  const { accessToken, role, authLoaded } = useSelector((state) => state.auth);
  //   const { authLoaded } = useLoading();
  const [showNotFound, setShowNotFound] = useState(false);

  //   useEffect(() => {
  //     // Only show NotFound after auth is fully loaded and we know the user isn't authenticated
  ...
  //     }
  //   }, [accessToken, role, requiredRole]);

  //   While auth is loading, show a loading indicator instead of NotFound
  //   if (!authLoaded) {
  //     return (
  ...
  // After auth is loaded, if user isn't authenticated, show NotFound
